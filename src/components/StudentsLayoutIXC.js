...
import { ixcStudents } from "../../data/ixc";
...
function StudentsLayoutIXC() {
  const [selectedFilters, setSelectedFilters] = useState([]);
  const [filteredStudents, setFilteredStudents] = useState(ixcStudents);
  ...
  const filterStudents = () => {
    if (selectedFilters.length > 0) {
      setFilteredStudents(
        ixcStudents.filter((student) =>
          selectedFilters.includes(student.subject)
        )
      );
    } else {
      setFilteredStudents([...ixcStudents]);
    }
    setLoading(false);
  };
...
export default StudentsLayoutIXC;
