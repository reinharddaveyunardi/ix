...
import { ixaStudents } from "../../data/ixa";
...
function StudentsLayoutIXA() {
  const [selectedFilters, setSelectedFilters] = useState([]);
  const [filteredStudents, setFilteredStudents] = useState(ixaStudents);
  ...
  const filterStudents = () => {
    if (selectedFilters.length > 0) {
      setFilteredStudents(
        ixaStudents.filter((student) =>
          selectedFilters.includes(student.subject)
        )
      );
    } else {
      setFilteredStudents([...ixaStudents]);
    }
    setLoading(false);
  };
...
export default StudentsLayoutIXA;
